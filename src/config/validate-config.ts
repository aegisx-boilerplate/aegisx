...
import { configSchema, enhancedConfigSchema } from './schema';
...
const result2 = configSchema.safeParse(envWithStrings);
if (result2.success) {
...
const result3 = configSchema.safeParse(invalidEnv);
if (!result3.success) {
...
const result4 = enhancedConfigSchema.safeParse(prodEnv);
if (result4.success) {
...
const result5 = enhancedConfigSchema.safeParse(badProdEnv);
if (!result5.success) {
...
const result6 = configSchema.safeParse(formatTest);
if (!result6.success) {
